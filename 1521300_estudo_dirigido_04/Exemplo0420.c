...
void method_20(void) {
    ...
    int controle = 0;
    int n;
    int total = 0, quantidadeAtual = 0;

    printf("Quantas cadeias de caractere deseja digitar: ");
    scanf("%d", &n);
    ...
    for (controle = 0; controle < n; controle++) {
        ...
        fgets(palavra, 100, stdin);
        quantidadeAtual = contALF(palavra);
        total += quantidadeAtual;
        printf("A palavra digitada tem um total de simbolos alfanumericos igual a: %d\n", quantidadeAtual);
    }

    printf("O total acumulado de caracteres de todas palavras e: %d\n", total);
}
